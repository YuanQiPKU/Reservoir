...
void account_item::change_time(Time_ t) {
   Transaction* new_account_item_message=new Transaction(account_name,t,account_kind,account_money);
  account_time = t;
...
void account_item::change_name(QString name) {
    Transaction* new_account_item_message=new Transaction(name,account_time,account_kind,account_money);
  account_name = name;
...
void account_item::change_money(double money) {
     Transaction* new_account_item_message=new Transaction(account_name,account_time,account_kind,money);
  account_money = money;
...
void account_item::change_kind(Kind kind) {
     Transaction* new_account_item_message=new Transaction(account_name,account_time,kind,account_money);
  account_kind = kind;
...
void account_item::make_sure(bool make){
    make_sure_or=make;
}
void account_item::on_btnRemove_clicked() {
    ...
    // 使用局部连接避免信号槽失效
    connect(remove_make_sure_widget, &remove_make_sure::make_sure_or, this, [this](bool make_sure) {
        if (make_sure) {
