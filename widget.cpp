...
Widget::Widget(QWidget *parent)
  ...
  QTabBar *tabBar = all_page->tabBar();
    connect(all_page, &QTabWidget::tabCloseRequested, this, &Widget::closeTab); // 设置tab可以被关闭
// 设置标签页样式
    tabBar->setStyleSheet("QTabBar::tab{color: white;background-color: rgba(61, 203, 255, 175);min-width: 20ex;min-height: 5ex;   font-family: 幼圆; }");
    tabBar->setStyleSheet(tabBar->styleSheet() +"QTabBar::tab:selected{background-color: rgba(255, 179, 0, 175);margin-top: 2ex;}");
    tabBar->setStyleSheet(tabBar->styleSheet() +"QTabBar::tab { border-radius: 5px; }"); // 为标签页增加圆角
     tabBar->setStyleSheet(tabBar->styleSheet() +"QTabBar::tab { font: bold 12px; }");

    /*这三行代码展示了如何使用Qt的样式表（Style Sheet）来设置`QTabBar`控件的样式。下面是对每一行代码的解释：

1. **设置标签页的基本样式**：
   ```cpp
   tabBar->setStyleSheet("QTabBar::tab{color: white;background: #5c8dff;border: 1px solid #73b7ff;min-width: 20ex;min-height: 5ex;   font-family: 幼圆; }");
   ```
   - `QTabBar::tab`：选择器，用于定位`QTabBar`中的每个标签页（tab）。
   - `color: white;`：设置标签页上文本的颜色为白色。
   - `background: #5c8dff;`：设置标签页的背景颜色为指定的蓝色。
   - `border: 1px solid #73b7ff;`：设置标签页的边框为1像素宽，实线，颜色为另一种蓝色。
   - `min-width: 20ex;`：设置标签页的最小宽度为20个字符的宽度（ex是一个长度单位，通常等于当前字体的x高度）。
...
2. **设置选中标签页的样式**：
   ...
   - `margin-top: 2ex;`：设置选中的标签页的顶部外边距为2个字符的高度，这会使其相对于其他标签页突出显示。

3. **为所有标签页增加圆角**：
   ```cpp
   tabBar->setStyleSheet(tabBar->styleSheet() + "QTabBar::tab { border-radius: 5px; }");
   ```
   - 这行代码的工作原理与第二行类似，它将新的样式规则添加到现有的样式表中。
   - `border-radius: 5px;`：为所有标签页添加5像素的圆角效果，使得标签页的角落看起来更加平滑。
