...
/*QDate 是 Qt 框架中的一个类，用于表示一个日期，它不包含时间信息。QDate 可以用于创建、操作和比较日期。以下是一些基本的 QDate 功能：

创建日期：可以使用 QDate 的构造函数来创建一个日期对象，例如 QDate date(2024, 5, 21); 创建了一个表示 2024 年 5 月 21 日的日期对象。

日期操作：QDate 提供了多种方法来对日期进行操作，比如增加或减少天数（addDays）、获取当前日期（currentDate）、计算两个日期之间的天数差（daysTo）等。

日期比较：QDate 支持比较操作符，可以比较两个日期是否相等，或者哪个日期在前。

格式化和解析：QDate 可以与 QString 相互转换，允许你将日期格式化为字符串，或者从字符串解析日期。

范围和验证：QDate 可以检查日期是否有效（比如不存在的日期，如 2 月 30 日），以及是否在某个范围内。
